...
void PipelineManager::Create(Device* device, SwapChain* swapChain, VkDescriptorSetLayout perFrameSetLayout, VkDescriptorSetLayout perObjectSetLayout, VkDescriptorSetLayout textureSetLayout, VkDescriptorSetLayout normalMapSetLayout, VkDescriptorSetLayout heightMapSetLayout, VkDescriptorSetLayout lightSetLayout)
{
    ...
    VkDescriptorSetLayout setLayouts[] = {perFrameSetLayout, perObjectSetLayout, textureSetLayout, normalMapSetLayout, heightMapSetLayout, lightSetLayout};
    ...
    auto swapchainFormat = swapChain->GetSwapChainImageFormat();
...
VkShaderModule PipelineManager::CreateShaderModule(const std::vector<uint32_t> &code, Device* device)
{
...
void PipelineManager::CreateLinePipeline(Device* device, SwapChain* swapChain)
{
    ...
    if (vkCreateGraphicsPipelines(device->GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_lineGraphicsPipeline) != VK_SUCCESS)
    {
...
void PipelineManager::CreateGBufferPipeline(Device* device, SwapChain* swapChain, VkDescriptorSetLayout perFrameSetLayout, VkDescriptorSetLayout perObjectSetLayout, VkDescriptorSetLayout textureSetLayout, VkDescriptorSetLayout normalMapSetLayout, VkDescriptorSetLayout heightMapSetLayout)
{
    auto vertShaderCode = ReadFile("shaders/vertex.spv");
    auto fragShaderCode = ReadFile("shaders/gbuffer.spv");
    ...
    VkDescriptorSetLayout setLayouts[] = { perFrameSetLayout, perObjectSetLayout, textureSetLayout, normalMapSetLayout, heightMapSetLayout };
    ...
    if (vkCreatePipelineLayout(device->GetDevice(), &pipelineLayoutInfo, nullptr, &m_gbufferPipelineLayout) != VK_SUCCESS)
        throw std::runtime_error("failed to create g-buffer pipeline layout!");

    VkFormat gbufferFormats[4] = {
        swapChain->GetPositionFormat(),
        swapChain->GetNormalFormat(),
        swapChain->GetAlbedoFormat(),
        swapChain->GetMaterialFormat()
    };
    VkFormat depthFormat = swapChain->GetDepthFormat();
    ...
    renderingInfo.pColorAttachmentFormats = gbufferFormats;
    ...
    if (vkCreateGraphicsPipelines(device->GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_gbufferPipeline) != VK_SUCCESS)
...
void PipelineManager::CreateLightingPipeline(Device* device, SwapChain* swapChain, VkDescriptorSetLayout perFrameSetLayout, VkDescriptorSetLayout gbufferSetLayout, VkDescriptorSetLayout lightSetLayout, VkDescriptorSetLayout shadowSetLayout)
{
    ...
    VkFormat lightingFormats[2] = {
        swapChain->GetLightingResultFormat(),
        swapChain->GetEmissiveAccumFormat()
    };

    VkPipelineRenderingCreateInfo renderingInfo{};
    renderingInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO;
    renderingInfo.colorAttachmentCount = 2;
    renderingInfo.pColorAttachmentFormats = lightingFormats;
    ...
    if (vkCreateGraphicsPipelines(device->GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_lightingPipeline) != VK_SUCCESS)
...
void PipelineManager::CreateCompositePipeline(Device* device, SwapChain* swapChain, VkDescriptorSetLayout compositeSetLayout)
{
    auto vertShaderCode = ReadFile("shaders/composite_vert.spv");
    auto fragShaderCode = ReadFile("shaders/composite_frag.spv");
    ...
    VkDescriptorSetLayout setLayouts[] = { compositeSetLayout, compositeSetLayout };
    ...
    if (vkCreatePipelineLayout(device->GetDevice(), &pipelineLayoutInfo, nullptr, &m_compositePipelineLayout) != VK_SUCCESS)
        throw std::runtime_error("failed to create composite pipeline layout!");

    VkFormat hdrFormat = swapChain->GetHdrColorFormat();
    ...
    if (vkCreateGraphicsPipelines(device->GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_compositePipeline) != VK_SUCCESS)
...
void PipelineManager::CreateTonemapPipeline(Device* device, SwapChain* swapChain, VkDescriptorSetLayout hdrSetLayout, VkDescriptorSetLayout perFrameSetLayout)
{
    auto vertShaderCode = ReadFile("shaders/tonemap_vert.spv");
    ...
    VkDescriptorSetLayout setLayouts[] = { hdrSetLayout, perFrameSetLayout };
    ...
    if (vkCreatePipelineLayout(device->GetDevice(), &pipelineLayoutInfo, nullptr, &m_tonemapPipelineLayout) != VK_SUCCESS)
    ...
    if (vkCreateGraphicsPipelines(device->GetDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_tonemapPipeline) != VK_SUCCESS)
...
void PipelineManager::Shutdown(Device* device)
{
    ...
    {
        vkDestroyPipeline(device->GetDevice(), m_compositePipeline, nullptr);
        m_compositePipeline = VK_NULL_HANDLE;
    }
    if (m_compositePipelineLayout != VK_NULL_HANDLE)
    {
        vkDestroyPipelineLayout(device->GetDevice(), m_compositePipelineLayout, nullptr);
        m_compositePipelineLayout = VK_NULL_HANDLE;
    }
    if (m_lightingPipeline != VK_NULL_HANDLE)
    {
        vkDestroyPipeline(device->GetDevice(), m_lightingPipeline, nullptr);
        m_lightingPipeline = VK_NULL_HANDLE;
    }
    if (m_lightingPipelineLayout != VK_NULL_HANDLE)
    {
        vkDestroyPipelineLayout(device->GetDevice(), m_lightingPipelineLayout, nullptr);
        m_lightingPipelineLayout = VK_NULL_HANDLE;
    }
    if (m_gbufferPipeline != VK_NULL_HANDLE)
    {
        vkDestroyPipeline(device->GetDevice(), m_gbufferPipeline, nullptr);
        m_gbufferPipeline = VK_NULL_HANDLE;
    }
    if (m_gbufferPipelineLayout != VK_NULL_HANDLE)
    {
        vkDestroyPipelineLayout(device->GetDevice(), m_gbufferPipelineLayout, nullptr);
        m_gbufferPipelineLayout = VK_NULL_HANDLE;
    }
    if (m_lineGraphicsPipeline != VK_NULL_HANDLE)
    {
        vkDestroyPipeline(device->GetDevice(), m_lineGraphicsPipeline, nullptr);
        m_lineGraphicsPipeline = VK_NULL_HANDLE;
    }
    ...
    {
        vkDestroyPipeline(device->GetDevice(), m_tonemapPipeline, nullptr);
...
std::vector<uint32_t> PipelineManager::ReadFile(const std::string &filename)
{
